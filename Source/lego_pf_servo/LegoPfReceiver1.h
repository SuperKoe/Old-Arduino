#ifndef LegoPfReceiver1_h
#define LegoPfReceiver1_h
...
class LegoPfReceiver1
{
  private:
    uint32_t elapsedSince(uint32_t since, uint32_t now);
    uint32_t elapsedSince(uint32_t since);
    uint32_t irRecvSignal();
    uint8_t PIN_IR;
    uint8_t PIN_LED;
    uint8_t led_count;
    uint32_t isrLastTimeStamp;
       
  public:   
    void setIRpin(uint8_t IR);
...
extern LegoPfReceiver1 LEGOPFRECEIVER1;
